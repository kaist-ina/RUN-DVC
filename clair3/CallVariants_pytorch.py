...
from clair3.model_Pytorch import *
...
def possible_outcome_probabilites_from_no_zygosity(
        gt21_probabilities,
        genotype_probabilities,
        variant_length_probabilities_1,
        variant_length_probabilities_2,
    ...
    if not add_indel_length:
        homo_Ref_probability = ( gt21_probabilities[reference_gt21]
                                )
        homo_SNP_probabilities = [ gt21_probabilities[gt21]
                                  for gt21 in HOMO_SNP_GT21]
        hetero_SNP_probabilities = [ gt21_probabilities[gt21]
                                    for gt21 in HETERO_SNP_GT21]
        if  gt21_probabilities[
            reference_gt21] >= 0.5:
            return [homo_Ref_probability]
        # Insertion
        homo_Ins_probabilities = [ gt21_probabilities[GT21_Type.InsIns]]
        homo_Ins_lengths = []
        hetero_InsIns_probabilities = [ gt21_probabilities[GT21_Type.InsIns]]
        hetero_InsIns_length_tuples = []
        hetero_ACGT_Ins_probabilities = []
        gt21_base_tuples = [(GT21_Type.AIns, "A"), (GT21_Type.CIns, "C"), (GT21_Type.GIns, "G"), (GT21_Type.TIns, "T")]
        for gt21, hetero_base in gt21_base_tuples:
            hetero_ACGT_Ins_probabilities.append(gt21_probabilities[gt21] )
        hetero_ACGT_Ins_bases, hetero_ACGT_Ins_lengths = [], []

        # Deletion
        homo_Del_probabilities = [ gt21_probabilities[GT21_Type.DelDel]]
        homo_Del_lengths = []

        hetero_DelDel_probabilities = [ gt21_probabilities[GT21_Type.DelDel]]
        hetero_DelDel_length_tuples = []

        hetero_ACGT_Del_probabilities = []
        gt21_base_tuples = [(GT21_Type.ADel, "A"), (GT21_Type.CDel, "C"), (GT21_Type.GDel, "G"), (GT21_Type.TDel, "T")]
        for gt21, hetero_base in gt21_base_tuples:
            hetero_ACGT_Del_probabilities.append(gt21_probabilities[gt21] 
                                                 )
        hetero_ACGT_Del_bases, hetero_ACGT_Del_lengths = [], []

        # InsDel
        hetero_InsDel_probabilities = [ gt21_probabilities[GT21_Type.InsDel]]
        ...
        reference_gt21 = gt21_enum_from_label(reference_base + reference_base)
        homo_Ref_probability = (
                variant_length_0_probability *  gt21_probabilities[reference_gt21]
        )
        if variant_length_0_probability_1 >= 0.5 and variant_length_0_probability_2 >= 0.5  and \
                gt21_probabilities[
                    reference_gt21] >= 0.5:
            return [homo_Ref_probability]

        homo_SNP_probabilities = [(
                variant_length_0_probability *  gt21_probabilities[gt21]
        ) for gt21 in HOMO_SNP_GT21]
        hetero_SNP_probabilities = [(
                variant_length_0_probability  * gt21_probabilities[gt21]
        ) for gt21 in HETERO_SNP_GT21]
        ...
        homo_Ins_lengths, homo_Ins_probabilities = zip(*homo_Ins_tuples_from(
            variant_length_probabilities_1, variant_length_probabilities_2,
             gt21_probabilities[GT21_Type.InsIns]
        ))
        homo_Ins_lengths, homo_Ins_probabilities = list(homo_Ins_lengths), list(homo_Ins_probabilities)
        hetero_InsIns_length_tuples, hetero_InsIns_probabilities = zip(*hetero_InsIns_tuples_from(
            variant_length_probabilities_1, variant_length_probabilities_2,
        ...
        homo_Del_lengths, homo_Del_probabilities = zip(*homo_Del_tuples_from(
            variant_length_probabilities_1, variant_length_probabilities_2,
             gt21_probabilities[GT21_Type.DelDel]
        ))
        homo_Del_lengths, homo_Del_probabilities = list(homo_Del_lengths), list(homo_Del_probabilities)
        hetero_DelDel_length_tuples, hetero_DelDel_probabilities = zip(*hetero_DelDel_tuples_from(
            variant_length_probabilities_1, variant_length_probabilities_2,
        ...
        hetero_InsDel_length_tuples, hetero_InsDel_probabilities = zip(*hetero_InsDel_tuples_from(
            variant_length_probabilities_1, variant_length_probabilities_2,
             gt21_probabilities[GT21_Type.InsDel]
        ))
        hetero_InsDel_length_tuples, hetero_InsDel_probabilities = (
            list(hetero_InsDel_length_tuples), list(hetero_InsDel_probabilities)
        )
...
def output_from(
    ...
    # gt21_probabilities = list(gt21_probabilities)
    # genotype_probabilities = list(genotype_probabilities)
    # variant_length_probabilities_1= list(variant_length_probabilities_1)
    # variant_length_probabilities_2 = list(variant_length_probabilities_2)
...
def batch_output_for_ensemble(X, batch_chr_pos_seq, alt_info_list, batch_Y, output_config, output_utilities):
    ...
    tensor_position_center = param.input_flankingBaseNum
...
def batch_output_rundmc(batch_chr_pos_seq, alt_info_list, batch_Y, output_config, output_utilities):
    batch_size = len(batch_chr_pos_seq)

    batch_gt21_probabilities, batch_genotype_probabilities = list(batch_Y[:,:param.label_shape_cum[0]]), list(batch_Y[:,param.label_shape_cum[0]:param.label_shape_cum[1]])
    if len(batch_gt21_probabilities) != batch_size:
        sys.exit(
            "Inconsistent shape between input tensor and output predictions %d/%d" %
            (batch_size, len(batch_gt21_probabilities))
    ...
    if output_config.add_indel_length:
        batch_variant_length_probabilities_1, batch_variant_length_probabilities_2 = list(batch_Y[:,param.label_shape_cum[1]:param.label_shape_cum[2]]), list(batch_Y[:,param.label_shape_cum[2]:param.label_shape_cum[3]])
    
    def calculate_quality(zyg):
        ...
        if reference > mut:
            return True, quality_score_from(reference)
        else:
            try:
                return False, quality_score_from(mut)
            except:
                print("ERROR!",mut)
                assert(False)
    ...
    for (
        ...
        # numpy byte, byte, ndarray, ndarray, ndarray
        # print(type(chr_pos_seq), type(alt_info), type(gt21_probabilities), type(genotype_probabilities), type(variant_length_probabilities_1))
        ...
        tensor_position_center = param.input_flankingBaseNum if len(reference_sequence) > 1 else 0
        reference_base_ACGT = BASE2ACGT[reference_sequence[tensor_position_center]]
        ...
        is_reference, quality_score = calculate_quality(genotype_probabilities)
...
def output_with(
    ...
    if len(reference_sequence) == 65: # youngmok: for ont with input size 65 in column (for AE or SSL)
        reference_sequence = reference_sequence[16:-16]
...
class  TorchDataset(Dataset):
    def __init__(self, dataset_fn,  chunk_start_pos, chunk_end_pos):
        import tables
        self.dataset = tables.open_file(dataset_fn, 'r').root
        # self.dataset = dataset
        self.chunk_start_pos = chunk_start_pos
        self.chunk_end_pos = chunk_end_pos
    
    def __len__(self):
        return self.chunk_end_pos - self.chunk_start_pos
    def __getitem__(self, idx):
        start_pos = self.chunk_start_pos + idx 
        end_pos = min(self.chunk_start_pos + (idx + 1) , self.chunk_end_pos)
        position_matrix = self.dataset.position_matrix[start_pos:end_pos]
        position = self.dataset.position[start_pos:end_pos]  # .flatten()
        alt_info_list = self.dataset.alt_info[start_pos:end_pos]  # .flatten()
...
def call_variants(args, output_config, output_utilities):
    ...
    if args.pileup:
        import shared.param_p as param
        from clair3.model_Pytorch import Clair3_P
        m = Clair3_P(add_indel_length=args.add_indel_length, predict=True)
    else:
        import shared.param_f as param

        m = RUNDVC([1,1,1], platform=args.platform, add_indel_length=args.add_indel_length, predict=True)
    print("Loaded Pretrained Model")
    checkpoint = torch.load(args.chkpnt_fn, map_location=torch.device('cpu'))
    ...
    batch_output_method = batch_output_rundmc if args.rundmc else batch_output_method
    mini_batches_loaded = []
    ...
    if not args.is_from_tables:
        ...
        if apply_threading:
                ...
                while True:
                    ...
                    if len(mini_batches_to_output) > 0:
                        ...
                        if args.pileup:
                            if X.shape[1] ==65:
                                X = X[:,16:-16,:]
                            input = torch.Tensor(X)/ param.NORMALIZE_NUM
                        else:
                            if X.shape[2] ==65:
                                X = X[:,:,16:-16,:]
                            input = torch.Tensor(X).permute(0,3,1,2) / param.NORMALIZE_NUM

                            if args.platform=="ilmn":
                                input = np.delete(input,[ 7],axis=1)
                        prediction = m( input )

                        if type(prediction) == type({'key':1}):
                            prediction = torch.cat([prediction['task1'], prediction['task2'], prediction['task3'], prediction['task4']], axis=1 )
                        total += len(X)
                        thread_pool.append(Thread(
                            target=batch_output_method,
                            args=(position, alt_info_list, prediction.detach().numpy(), output_config, output_utilities)
                        ))
            ...
            while True:
                if len(mini_batches_to_output) > 0:
                    ...
                    X, position, alt_info_list = mini_batch
                    input = torch.Tensor(X).permute(0,3,1,2) / param.NORMALIZE_NUM
                    # Youngmok: Tensorflow normalize inside the model, but torch assumes input is normalized
                    prediction = m( input )
                    if type(prediction) == type({'key':1}):
                        prediction = torch.cat([prediction['task1'], prediction['task2'], prediction['task3'], prediction['task4']], axis=1 )
                    total += len(X)
                    batch_output_method(position, alt_info_list, prediction.detach().numpy(), output_config, output_utilities)
        ...
        with torch.no_grad():
            ...
            num_empty = 0
            for idx in range(num_epoch):
                print("{} / {}, 1-step took:{}".format(idx, num_epoch, time() - start))
                start = time()
                ...
                if USE_PREDICTION:                 
                    ...
                    if args.pileup:
                        if position_matrix.shape[1] ==65:
                            position_matrix = position_matrix[:,16:-16,:]
                    else:
                        if position_matrix.shape[2] ==65:
                            position_matrix = position_matrix[:,:,16:-16,:]



                    if args.pileup:
                        input = torch.Tensor(position_matrix) / param.NORMALIZE_NUM
                    else:
                        input = torch.Tensor(position_matrix).permute(0,3,1,2) / param.NORMALIZE_NUM
                        # Youngmok: delete channel 5,7 candidate proportion, phasing info
                        # input = np.delete(input,[2,3,4,5,7],axis=1)
                        if args.platform=="ilmn":
                            input = np.delete(input,[ 7],axis=1)
                    ...
                    if position_matrix.shape[2] ==65:
                        position_matrix = position_matrix[:,:,16:-16,:]
                    label_list = dataset.label[chunk_start_pos + idx * batch_size:chunk_start_pos + (idx + 1) * batch_size]

                    if label_list.shape[1] == 154:
                        label = np.split(label_list,list(accumulate([21,3,65,65])), 1 )[0:4]
                        
                        ins_sums = np.sum(label[2][:,-16:],axis=1)
                        del_sums = np.sum(label[2][:,:16],axis=1)

                        label[2] = label[2][:, 16:-16]
                        
                        for idx,i in enumerate(ins_sums):
                            if i != 0:
                                label[2][idx,-1] = 1
                        for idx,i in enumerate(del_sums):
                            if i != 0:
                                label[2][idx,0] = 1

                        ins_sums = np.sum(label[3][:,-16:],axis=1)
                        del_sums = np.sum(label[3][:,:16],axis=1)

                        label[3] = label[3][:, 16:-16]
                ...
                # youngmok: in batch_output, change the numpy to list for faster iteration, it can be really slow
...
def predict(args, output_config, output_utilities):
    ...
    if args.pileup:
        ...
        from clair3.model_Pytorch import RUNDVC, RUNDVC_usingFeature
        from esm.model import ViT, ViT_axial
        # m = RUNDVC([1,1,1],add_indel_length=args.add_indel_length, predict=True)
        m = RUNDVC_usingFeature([1,1,1],add_indel_length=args.add_indel_length, predict=True)
        # m = ViT( patch_size=(5,3), dim=256, depth=3, heads=8, mlp_dim=512 , channels = 8 )
    batch_output_method = batch_output_for_ensemble if output_config.is_output_for_ensemble else batch_output
    m.load_state_dict(torch.load(args.chkpnt_fn))
    m.eval()
    ...
    if not args.is_from_tables:
        ...
        predict_dataset = DataLoader(DataGenerator(dataset, num_epoch, batch_size, chunk_start_pos, chunk_end_pos), batch=1,shuffle=False,num_workers=1, pin_memory=True)

        dataset_iter = iter(predict_dataset)
        for idx in range(num_epoch):
            position_matrix, position, alt_info_list = next(dataset_iter)
            prediction = m(position_matrix)
...
def main():
    ...
    parser.add_argument('--rundmc', action='store_true',
                        help=SUPPRESS)
