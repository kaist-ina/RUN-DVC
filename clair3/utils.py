...
def bin_reader_generator_from(tensor_fn, Y_true_var, Y, is_tree_empty, tree, miss_variant_set, truth_alt_dict, is_allow_duplicate_chr_pos=False, maximum_non_variant_ratio=None):
    ...
    tensor_fn: tensor file. youngmok: tensor file includes non-variant regions too
    ...
    for row_idx, row in enumerate(tensor_fn):
        # youngmok: in createtensorfullalignment.py, generate_tensorfunction 
        # Below corresponds to ctg_name, center_pos, ref_seq, tensor_string, alt_info
        ...
        if seq[param.flankingBaseNum] not in 'ACGT':
            continue
        key = chrom + ":" + coord
        # Youngmok: is_reference, not in truth VCF file
        is_reference = key not in Y_true_var

        if key in miss_variant_set:
            continue

        # Youngmok: check if there are supporting reads for the variant
        ...
        if len(X) == shuffle_bin_size:
            if maximum_non_variant_ratio is not None:
                print("[Pre-Filter]Num Data:",len(X),"Num ref", len(ref_list))
                _filter_non_variants(X, ref_list, maximum_non_variant_ratio)
                print("[Post-Filter]Num Data:",len(X),"Num ref", len(ref_list))
            yield X, total, False
            X = {}
            ref_list = []
        total += 1
...
def get_training_array(tensor_fn, var_fn, ctgName, bed_fn, bin_fn, shuffle=True, is_allow_duplicate_chr_pos=True, chunk_id=None,
    ...
    tree = bed_tree_from(bed_file_path=bed_fn, contig_name=ctgName)
