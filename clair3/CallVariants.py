...
def call_variants(args, output_config, output_utilities):
    ...
    if not args.is_from_tables:
        ...
        if apply_threading:
            while True:
                ...
                if len(mini_batches_to_output) > 0:
                    ...
                    # print("1-step took:{}".format( time() - start))
        ...
        for idx in range(num_epoch):
            ...
            #     sums = np.sum(label[2],axis=1)
            #     for idx,i in enumerate(sums):
            #         if i ==0:
            #             label[2][idx,-1] = 1
            #             print(alt_info_list[idx])


            #     sums = np.sum(label[3],axis=1)
            #     for idx,i in enumerate(sums):
            #         if i ==0:
            #             label[3][idx,-1] = 1
            #             print(alt_info_list[idx])
            ...
            # indels =  np.argmax( prediction[:,:21], axis=1 ) 
            # for idx, id in enumerate(indels):
            #     # print("indel length:",np.argmax(prediction[idx][24:57]), np.argmax(label[idx][24:57]) )
            #     if np.argmax(label[idx][24:57]) < 16:
            #         if (10<= np.argmax(label[idx][:21]) and 14>= np.argmax(label[idx][:21])) or np.argmax(label[idx][:21]) == 20:

            #             print("indel length mall", np.argmax(label[idx][24:57]),  np.argmax(label[idx][:21]))
            #         else: 
            #             print("strange indel length mall", np.argmax(label[idx][24:57]),  np.argmax(label[idx][:21]))
            #     if id >= 10 and np.argmax(label[idx][:21]) == np.argmax(label[idx][:21]):
            #         if np.argmax(prediction[idx][24:57]) != np.argmax(label[idx][24:57]) and np.argmax(prediction[idx][57:]) != np.argmax(label[idx][57:]):
            #             print("Same genotype with different variant length")
            #             print(np.argmax(prediction[idx][24:57]) , np.argmax(label[idx][24:57]))
            #             print(np.argmax(prediction[idx][57:]), np.argmax(label[idx][57:]))
            # print("Prediction took:", time() - model_inf)
            batch_output_method(position, alt_info_list, prediction, output_config, output_utilities)
