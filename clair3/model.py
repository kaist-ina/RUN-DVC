...
class PyramidPolling(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        height = int(input_shape[1]) # 28
        width = int(input_shape[2]) # 17
    ...
    def call(self, x):
        for i in range(self.pool_len):
            max_pool = tf.keras.layers.MaxPool2D(pool_size=(self.window_h[i], self.window_w[i]), strides=(self.stride_h[i], self.stride_w[i]), padding='SAME')(x)
...
class BasicBlockTest(tf.keras.layers.Layer):

    def __init__(self, filter_num, stride=1,SeparableConv=False):
        super(BasicBlockTest, self).__init__()
        conv = tf.keras.layers.SeparableConv2D if SeparableConv else tf.keras.layers.Conv2D
        # stride is 1 
        self.conv1 = conv(filters=filter_num,
                                            kernel_size=(3, 3),
...
def make_basic_block_layerTest(filter_num, blocks, stride=1, SeparableConv=False):

    res_block = tf.keras.Sequential()

    res_block.add(BasicBlockTest(filter_num, stride=stride, SeparableConv=SeparableConv))

    for _ in range(1, blocks):
        res_block.add(BasicBlockTest(filter_num, stride=1,SeparableConv=SeparableConv))

    return res_block

class BasicConv2DTest(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size, strides, padding, SeparableConv=False):
        super(BasicConv2DTest, self).__init__()
        conv = tf.keras.layers.SeparableConv2D if SeparableConv else tf.keras.layers.Conv2D
        self.conv = conv(filters=filters,
                                           kernel_size=kernel_size,
                                           strides=strides,
    ...
    def call(self, inputs):
        ...
        return output
class OutputTest(tf.keras.Model):
    # Residual CNN model for clair3 full alignment input
    def __init__(self, add_indel_length=False, predict=False):
        super(OutputTest, self).__init__()
        self.output_gt21_shape = params['output_gt21_shape']
        self.output_genotype_shape = params['output_genotype_shape']
        self.output_indel_length_shape_1 = params['output_indel_length_shape_1']
        self.output_indel_length_shape_2 = params['output_indel_length_shape_2']
        ...
        self.conv1 = BasicConv2DTest(filters=8,
                                 kernel_size=(3, 3),
                                 strides=2,
                                 padding="same"
                                 )

        # self.res_block1 = make_basic_block_layerTest(filter_num=8,
        #                                     blocks=1, stride=1, SeparableConv=False)
        self.conv3 = BasicConv2DTest(filters=8,
                                 kernel_size=(3, 3),
                                 strides=1,
                                 padding="same")

        # self.res_block2 = make_basic_block_layerTest(filter_num=8,
        #                                     blocks=1, stride=1, SeparableConv=False)

        self.conv5 = BasicConv2DTest(filters=8,
                                 kernel_size=(3, 3),
                                 strides=2,
                                 padding="same")

        # self.res_block3 = make_basic_block_layerTest(filter_num=8,
        #                                     blocks=1, stride=1)
        self.pyramidpolling = PyramidPolling()
    ...
    def call(self, inputs):
        ...
        # x = self.res_block2(x)
        t1 = self.conv5(x)
        # t1 = self.res_block3(x)
        x = self.pyramidpolling(t1)
        return x, t1
...
def main():
    ...
    model = OutputTest( add_indel_length=True)
    model.build((None,55,33,8))
    # print(output)
    idx = 0

    weights = [ np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8), 
    np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8),
    # np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8),
    # np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8),
    # np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8),
    # np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8),
    # np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8),
    # np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8),
    np.transpose( np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3), (2,3,1,0) )/200000.0, -np.zeros(8), ]#, np.zeros( (3,3,8,8) ) - 0.5, np.zeros(8) - 0.5, np.zeros((3,3,8,8)) + 0.5, np.zeros(8) + 0.5 ]
    model.set_weights( weights )
    # model.kernel.assign(weights)

    test_input = np.moveaxis( np.arange(8*55*33, dtype=np.float32).reshape(-1,8,55,33) , 1,3 )
    output, out2 = model(tf.convert_to_tensor(test_input))

    output = output.numpy()
    out2 = out2.numpy()

    total_sum = 0
    print(np.sort(output,axis=1))
    print("\nDetail:",output.sum())
    output = out2
    if len(output.shape)>3:
        for i in range(output.shape[1]):
            for j in range(output.shape[2]):
                total_sum += sum(output[0,i,j,:])
                # print(sum(output[0,i,j,:]), output[0,i,j,:])
                # print(sum(output[0,i,j,:]))
                print(output[0,i,j,:])
