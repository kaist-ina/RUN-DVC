...
class BasicConv2D(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
# ResNet
class RUNDVC(nn.Module):
    def __init__(self,  layers, platform="ilmn", add_indel_length=False, predict=False):
        super(RUNDVC, self).__init__()
...
class RUNDVC_Feature(nn.Module):
    def __init__(self,  layers, platform="ilmn"):
        super(RUNDVC_Feature, self).__init__()
...
def main():
    ...
    model = OutputTest([1,1,1], add_indel_length=True)
    # print(output)
    idx = 0
    weights = [ np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/200000.0, -np.zeros(8),
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8),
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8),
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8),
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8),
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8),
    ...
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8),
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8),
    np.arange( 3*3*8*8, dtype=np.float32 ).reshape(8,8,3,3)/ 200000.0, -np.zeros(8) ]#, np.zeros( (3,3,8,8) ) - 0.5, np.zeros(8) - 0.5, np.zeros((3,3,8,8)) + 0.5, np.zeros(8) + 0.5 ]
    bb = [0, -0.5, 0.5]
    for name, param in model.named_parameters():
        
        print(name, param.shape)
        # nn.init.constant_(param, 3.5)
        # param.data = torch.Tensor( np.random.random( list(param.shape)) , )
        # if len(param.shape) > 2:
        #     print(param.shape)
        #     param.data = torch.Tensor( np.zeros( list(param.shape) ) +bb[idx//2] )
        # else:
        #     print(param.shape)
        #     param.data = torch.Tensor( np.zeros(8) + bb[idx//2] )
        param.data = torch.Tensor( weights[idx]  )
        idx += 1
    print(idx)
    test_input = np.arange(8*55*33, dtype=np.float32).reshape(-1,8,55,33)
    output, out2 = model(torch.from_numpy( test_input) )
    
    output = output.detach().numpy()
    out2 = out2.detach().numpy()

    total_sum = 0
    print(np.sort(output).reshape(1,-1))
    print("\nDetail:", output.sum())
    
    output = out2
    # print(out2[0,0,10:19,6:12])
    # print(output[:9])
