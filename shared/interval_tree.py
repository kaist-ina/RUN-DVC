...
def bed_tree_from(bed_file_path, expand_region=None, contig_name=None, bed_ctg_start=None, bed_ctg_end=None,
    ...
    for row_id, row in enumerate(unzip_process.stdout):
        ...
        ctg_name = columns[0]
        ctg_name =  "chr" + ctg_name if ctg_name and not ctg_name.startswith("chr") else  ctg_name
        contig_name =  "chr" + contig_name if contig_name and not contig_name.startswith("chr") else  contig_name

        if contig_name != None and ctg_name != contig_name and "chr"+ctg_name != contig_name:
            continue

        if contig_name != None and contig_name not in tree:
            tree[contig_name] = IntervalTree()
        elif contig_name == None and ctg_name not in tree: 
        ...
        tree[contig_name if contig_name != None else ctg_name].addi(ctg_start, ctg_end)
...
def is_region_in(tree, contig_name, region_start=None, region_end=None):
    if not tree or (contig_name is None) or (contig_name not in tree and "chr"+contig_name not in tree):
        return False
