...
if [ "${PLATFORM}" = "ilmn" ]
then
    echo "[INFO] 2/4 No phasing for full alignment calling"
    ...
    # --phasing_info_in_bam \
    echo $''
    echo "[INFO] 3/4 Call all variants using full-alignment model"
    ...
    "${PYTHON} ${CLAIR3} CallVarBam \
        ...
        --call_fn ${FULL_ALIGNMENT_OUTPUT_PATH}/full_alignment_{1}_{2}_${CHUNK_NUM}.vcf \
        --sampleName ${SAMPLE} \
        --extend_bed ${SPLIT_BED_PATH}/{1} \
        --ref_fn ${REFERENCE_FILE_PATH} \
        --ctgName {1} \
    ...
    # echo $''
    # echo "[INFO] 6/7 Call all variants using full-alignment model"
    # # cat ${CANDIDATE_BED_PATH}/FULL_ALN_FILE_* > ${CANDIDATE_BED_PATH}/FULL_ALN_FILES
    # time ${PARALLEL} --retries ${RETRIES} --joblog ${LOG_PATH}/parallel_6_call_var_bam_full_alignment.log -j ${THREADS_LOW} \
    # "${PYTHON} ${CLAIR3} CallVarBam \
    #     --chkpnt_fn ${FA_PREFIX} \
...
then
    echo 'Finished'
    cp ${OUTPUT_FOLDER}/full_alignment.vcf.gz ${OUTPUT_FOLDER}/merge_output.vcf.gz
    cp ${OUTPUT_FOLDER}/full_alignment.vcf.gz.tbi ${OUTPUT_FOLDER}/merge_output.vcf.gz.tbi
else
    echo $''
...
echo "${OUTPUT_FOLDER}/merge_output.vcf.gz"
