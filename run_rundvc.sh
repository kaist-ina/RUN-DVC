...
VERSION='v1'
Usage="Usage: ${SCRIPT_NAME} --rundvc_call_mut --bam_fn=BAM --ref_fn=REF --output=OUTPUT_DIR --threads=THREADS --platform=PLATFORM --pileup_model=PILEUP_MODEL [--bed_fn=BED] [options]"
...
print_help_messages()
{
    ...
    echo $'  --pileup_model=STR Model for pileup calling'
    echo $'  --fa_model=STR Model for full-alignment calling.'
    ...
    echo $'      --rundvc_call_mut         Call mutation with RUNDMC trained models.'
...
min_mq::,min_coverage::,min_contig_size::,snp_min_af::,indel_min_af::,pileup_model::,fa_model::,fast_mode,gvcf,pileup_only,rundvc_call_mut,print_ref_calls,haploid_precise,haploid_sensitive,include_all_ctgs,\
remove_intermediate_dir,no_phasing_for_fa,call_snp_only,enable_phasing,enable_long_indel,use_gpu,longphase_for_phasing,disable_c_impl,help,version -n 'run_rundvc.sh' -- "$@"`
...
while true; do
   case "$1" in
    ...
    --rundvc_call_mut ) RUNDMC_CALLMUT=True; shift 1 ;;
...
if [ -z ${BAM_FILE_PATH} ] || [ -z ${REFERENCE_FILE_PATH} ] || [ -z ${THREADS} ] || [ -z ${OUTPUT_FOLDER} ] || [ -z ${PLATFORM} ] ; then
      if [ -z ${BAM_FILE_PATH} ] && [ -z ${REFERENCE_FILE_PATH} ] && [ -z ${THREADS} ] && [ -z ${OUTPUT_FOLDER} ] && [ -z ${PLATFORM} ]; then print_help_messages; exit 0; fi
      ...
      if [ "${RUNDMC_CALLMUT}" == True ] && [ -z ${FA_PREFIX} ]; then echo -e "${ERROR} Require to define model path by --fa_model=FULLALIGNMENT_MODEL${NC}"; fi
      exit 1;
fi

if [ "${RUNDMC_CALLMUT}" == False ] && [ -z ${PILEUP_PREFIX} ]; then echo -e "${ERROR} Require to define model path by --pileup_model=PILEUP_MODEL${NC}"; exit 1; fi
if [ "${RUNDMC_CALLMUT}" == True ] && [ "${PLATFORM}" != "ilmn" ] && [ -z ${PILEUP_PREFIX} ]; then echo -e "${ERROR} Require to define model path by --pileup_model=PILEUP_MODEL${NC}"; exit 1; fi
...
if [ `pwd` = "/opt/bin" ]; then
    ...
    if [[ ! "${PILEUP_PREFIX}" = /* ]]; then echo -e "${ERROR} Require to use absolute file path --pileup_model=PATH${NC}"; exit 1; fi
    if [[ ! "${FA_PREFIX}" = /* ]]; then echo -e "${ERROR} Require to use absolute file path --pileup_model=PATH${NC}"; exit 1; fi
...
if [[ ! "${PILEUP_PREFIX}" = /* ]] && [ -d ${PILEUP_PREFIX} ]; then PILEUP_PREFIX=`pwd`/${PILEUP_PREFIX}; fi
if [[ ! "${FA_PREFIX}" = /* ]] && [ -d ${FA_PREFIX} ]; then FA_PREFIX=`pwd`/${FA_PREFIX}; fi
...
(time (
echo "[INFO] RUNDVC-CLAIR3 VERSION: ${VERSION}"
...
# if [ ! -f ${MODEL_PATH} ]; then echo -e "${ERROR} Model not found${NC}"; exit 1; fi

if [  ${PLATFORM} != "ilmn" ] || [ "${RUNDMC_CALLMUT}" != True ]; then
    if [ ! -f ${PILEUP_PREFIX} ]; then echo -e "${ERROR} Pileup Model not found${NC}"; exit 1; fi
fi

if [ "${RUNDMC_CALLMUT}" == True ]; then
    if [ ! -f ${FA_PREFIX} ]; then echo -e "${ERROR} Full alignment Model not found${NC}"; exit 1; fi
fi
...
if [ -z ${PILEUP_PREFIX} ]; then echo -e "${ERROR} Use '--pileup_model=STR' instead of '--pileup_model STR' for optional parameters${NC}"; exit 1 ; fi
if [ -z ${FA_PREFIX} ]; then echo -e "${ERROR} Use '--fa_model=STR' instead of '--fa_model STR' for optional parameters${NC}"; exit 1 ; fi
...
RUNDVC_SCRIPT="rundmc_selectunlabeled.sh"
if [ "${RUNDMC_CALLMUT}" == True ]; then RUNDVC_SCRIPT="rundmc_CallVariants.sh"; fi

set -x
${SCRIPT_PATH}/scripts/${RUNDVC_SCRIPT} \
