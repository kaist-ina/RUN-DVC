...
HAP_TYPE = dict(zip((1, 0, 2), (-50, 20, 50)))  # hap1 UNKNOWN H2

# ACGT_NUM = dict(zip("ACGT+-*#N", (100, 25, 75, 50, -50, -100, 0, 0, 100)))
ACGT_NUM = dict(zip("ACGT+-*#N", (75, -50, 50, -75, -25, 25, 0, 0, 75)))
# youngmok: added R,Y to 100
# ACGT_NUM = dict(zip("ACGT+-*#NRY", (100, 25, 75, 50, -50, -100, 0, 0, 100, 100, 100)))
...
class Position(object):
    def __init__(self, pos, ref_base=None, alt_base=None, read_name_list=None, base_list=None, raw_base_quality=None,
                 raw_mapping_quality=None, af=None, depth=None, genotype=None, phase_set=None, read_map_pos=None):
        self.pos = pos
        self.ref_base = ref_base
        self.alt_base = alt_base
        self.read_name_list = read_name_list
        # youngmok: add read_map_pos for strand info
        self.read_map_pos = read_map_pos
    ...
    def update_infos(self):
        ...
        # youngmok: read_name is sorted by its pileup order
        for read_name, base_info, bq, mq in zip(self.read_name_list, self.base_list, self.base_quality,
            ...
            # youngmok: at pos, read_channel info of read_name, if read is not aligned, 0s
...
def sorted_by_hap_read_name(center_pos, haplotag_dict, pileup_dict, hap_dict, platform):
    ...
    # youngmok: remove dup names, preserve the pos-based sort order
    ...
    # youngmok: random sample if depth is too much
    ...
    # youngmok: add hap info if given by (need_phasing or phasing_info_in_bam)
...
def get_tensor_info(base_info, bq, ref_base, read_mq, read_mp=0, pos=0):
    """
    ...
    ins_base = ""
    query_base = ""
    # youngmok: channel size is 8 for FA
    read_channel = [0] * channel_size
    if base[0] in '*#':
        read_channel[3] = read_mq # to differentiate position where is not aligned or deletion.
        return read_channel, ins_base, query_base
    # assert(pos >= read_mp)

    strand = STRAND_1 #int(STRAND_1 * (0.1 + 0.9*(pos-read_mp)/300 ))
    if base[0] in 'ACGT':
        strand = STRAND_0 #int(STRAND_0 * (0.1 + 0.9*(pos-read_mp)/300 ))
    ALT_BASE = 0
    # youngmok: ACGT_NUM = dict(zip("ACGT+-*#N", (100, 25, 75, 50, -50, -100, 0, 0, 100)))
...
def generate_tensor(ctg_name, center_pos, sorted_read_name_list, pileup_dict, ref_seq, reference_sequence,
    ...
    # youngmok: alt dict keeps the indel info
    ...
    if not pass_confident_bed:
        return None, None
    # youngmok: start_pos = center - flanking base , end_pos ...
    for p in range(start_pos, end_pos):
        ...
        for read_idx, read_name_info in enumerate(sorted_read_name_list):
            ...
            if p in pileup_dict and read_name in pileup_dict[p].read_info:
                # youngmok: see get_tensor_info, read_channel, contains
                read_channel, ins_base = pileup_dict[p].read_info[read_name]
                # youngmok: read_channel is 5 info at position, for the read [read_idx][position]
                tensor[read_idx][offset] = read_channel
                tensor[read_idx][offset][5] = 0
                # assert(tensor[read_idx][offset][5] == 0)
                if ins_base != '' and p < end_pos - 1:
                    insert_tuple.append((read_idx, offset, ins_base, p))
    # Youngmok: [see get_tensor_info func] REF_BASE, ALT_BASE, strand, read_mq, bq is filled in from upper for loop
    # Youngmok: Fill in insertion information in 7th channel
    ...
    # Youngmok: Fill in candidate, phase information in 6, 8th channel
    for row_idx, (hap, _, read_name) in enumerate(sorted_read_name_list):
        ...
        if read_name in pileup_dict[center_pos].read_name_dict:
            ...
            base_upper = base.upper()
            if indel != '':
                # youngmok: calculate the frequency of indel
                if indel[0] == '+':
        ...
        # youngmok: mark only the position of the mutation
        # if tensor[row_idx][flanking_base_num][2] != 0:  # skip all del #*
        tensor[row_idx][flanking_base_num][5] = af_num
...
def CreateTensorFullAlignment(args):
    ...
    global test_pos
    test_pos = None
    # hete_snp_pos not used for illumina
    ...
    if full_aln_regions:
        ...
        for row in candidate_file_path_output:
            ...
            if len(row) > 3:  # hete snp positions
                ...
                # youngmok: if need_phasing_pos_set is not empty, samtools mpileup outputs only below center positions
    ...
    if not full_aln_regions and chunk_id is not None:
        ...
        if False:
            ...
            # youngmok: check the selected chromosomes' length, obtain contig_length from it, get chunk_size
            contig_length = 0
            with open(fai_fn, 'r') as fai_fp:
                for row in fai_fp:
                    ...
                    if contig_name != ctg_name and "chr"+contig_name != ctg_name:
                        continue
    ...
    if need_phasing and phased_vcf_fn and os.path.exists(phased_vcf_fn):
        ...
        for row in unzip_process.stdout:
            ...
            columns = row.strip().split('\t')
            contig_name = columns[0]
            if ctg_name and contig_name != ctg_name and "chr"+contig_name != ctg_name:
                continue
            pos = int(columns[1])
    ...
    # dont need to set bed_option if RR is used infront with extend_bed option, it produces bed regions only
    
    bed_option = ' -l {}'.format(
        extend_bed) if is_extend_bed_file_given else "" # and (platform != 'ilmn' or (platform == 'ilmn' and unify_repre)) else ""
    # youngmok: if full_aln_regions is given, set bed_options to full_aln_regions
    ...
    # youngmok: phasing_window_size is 10000 in param_f.py
    ...
    def samtools_pileup_generator_from(samtools_mpileup_process):
        ...
        for row in samtools_mpileup_process.stdout:  # chr position N depth seq BQ read_name mapping_quality phasing_info
            ...
            # youngmok: ignore positions that are lower than reference start + flanking_base 
            if is_ctg_range_given and platform == "ilmn" and (pos < reference_start + flanking_base_num or pos > reference_end - flanking_base_num+1):
                continue
            ...
            # youngmok: skip position not in range (range includes region around variant candidate, or given chromosome)
            ...
            # youngmok: phasing info in bam, column 8 in mpileup
            ...
            if phasing_info_in_bam:
                ...
                if len(read_name_list) != len(phasing_info):
                    ...
                    for hap_idx, hap in enumerate(phasing_info):
                        # youngmok: if not given hap is set to *, therefore nothing is added to hap_dict
                        if hap in '12' and read_name_list[hap_idx] not in hap_dict:
                            hap_dict[read_name_list[hap_idx]] = int(hap)
            # youngmok: does this happen?
            ...
            # add condition to remove  reference cases
            if is_known_vcf_file_provided and not has_pileup_candidates and pos in known_variants_set and (
                    pass_af and depth >= min_coverage):
                need_phasing_pos_list.append(pos)
            ...
            # youngmok: extend_bp_distance is the distance to find snps near need_phasing_pos 
    ...
    for hete_pos in hete_snp_pos_dict:
        if need_phasing and hete_snp_pos_dict[hete_pos].ref_seq is None:
            # youngmok: save alt sequence in alt_seq
    ...
    while True:
        ...
        # print(pos, reference_start, len(reference_sequence))
