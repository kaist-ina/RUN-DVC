...
def samtools_view_generator_from(samtools_view_process, aligned_reads, pileup, ctg_name, reference_sequence,
    ...
    for row_id, row in enumerate(samtools_view_process.stdout):
        ...
        RNAME = columns[2]
        if RNAME != ctg_name and "chr"+RNAME != ctg_name:
            continue
                ...
                for _ in range(advance):
                    if QUAL[query_position] >= min_dbg_base_quality:
                        try:
                            ...
                            print("ERROR",ctg_name, reference_position, reference_start_0_based, len(reference_sequence))
                            exit(1)
...
def reads_realignment(args):
    ...
    if is_ctg_range_given:
        ...
        extend_end = 1 if extend_end < 1 else extend_end
        ...
        # print("Contig name and range is given:",extend_start, reference_start, extend_end, reference_end, reads_regions)
    ...
    if read_fn and read_fn == 'PIPE':
        ...
        save_file_fp = subprocess_popen(shlex.split("{} view -bh - -o {}".format(samtools_execute_command, read_fn + (
            '.{}_{}'.format(args.chunk_id, chunk_num) if args.chunk_id and chunk_num  else ""))), stdin=PIPE,
                                        stdout=PIPE)
