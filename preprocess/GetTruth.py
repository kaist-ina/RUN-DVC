...
def OutputVariant(args):
    ...
    for row in vcf_fp.stdout:
        ...
        if chromosome != ctg_name and "chr"+chromosome != ctg_name:
            continue
        ...
        var_fp.stdin.write(" ".join((ctg_name, position, reference, alternate, genotype_1, genotype_2)))
        var_fp.stdin.write("\n")

    for position in truth_vcf_set:
        if position not in variant_set:
            # miss variant set used in Tensor2Bin
            var_fp.stdin.write(" ".join((ctg_name, str(position), "None", "None", "-1", "-1")))
            var_fp.stdin.write("\n")
