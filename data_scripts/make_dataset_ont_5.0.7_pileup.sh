...
${PARALLEL} -j${THREADS} ln -sf {4} ${PHASE_BAM_PATH}/{2}_{3}_{1}.bam ::: ${CHR[@]} ::: ${ALL_SAMPLE[@]} :::+ ${DEPTHS[@]} :::+ ${ALL_UNPHASED_BAM_FILE_PATH[@]}
${PARALLEL} --retries ${RETRIES} -j${THREADS} ln -sf {4}.bai ${PHASE_BAM_PATH}/{2}_{3}_{1}.bam.bai ::: ${CHR[@]} ::: ${ALL_SAMPLE[@]} :::+ ${DEPTHS[@]} :::+ ${ALL_UNPHASED_BAM_FILE_PATH[@]}
